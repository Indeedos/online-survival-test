// v0.8.0 — persist semantic answer identity so randomized option order cannot corrupt restores/reviews
(function(){
  const dirty=new Set();
  ...
  function enrich(task,a){
    const meta=integrityFor(task,a);
    if(meta)a._integrity=meta;
  }
  function remap(task,a){
    const meta=a?._integrity;
    if(!task||!a||meta?.v!==1||!Array.isArray(meta.selectedIds))return;
    const ids=currentIds(task);
    const mapped=meta.selectedIds.map(id=>ids.indexOf(id)).filter(i=>i>=0);
    if(task.t==='multi')a.choices=mapped;
    else a.choice=mapped.length?mapped[0]:null;
  }
  ...
  document.addEventListener('change',e=>{
    if(e.target?.matches?.('#answers input[name="q"]')&&typeof i==='number')dirty.add(i);
  },true);

  if(typeof window.render==='function'){
    const previousRender=window.render;
    window.render=function(){
      try{const task=items?.[i],a=answers?.[i];if(isChoiceTask(task)&&a?._integrity)remap(task,a)}catch{}
  ...
  if(typeof window.save==='function'){
    ...
    window.save=function(){
      ...
      const task=idx!=null?items?.[idx]:null;
      const before=idx!=null?answers?.[idx]:null;
      const hadIntegrity=!!before?._integrity;
      const wasLegacy=!!before&&isChoiceTask(task)&&!hadIntegrity;
      const userChanged=idx!=null&&dirty.has(idx);
      const result=previousSave.apply(this,arguments);
      try{
        if(idx!=null&&isChoiceTask(task)){
          // Never silently "upgrade" a restored legacy numeric answer: its original
          // randomized order is unknowable. Only new/explicitly changed answers get trusted metadata.
          if(wasLegacy&&!userChanged){answers[idx]=before}
          else enrich(task,answers?.[idx]);
          dirty.delete(idx);
        }
      }catch{}
      return result;
  ...
  window.__answerIntegrity={
    currentIds,
    remap,
    isChoiceTask,
    state(task,value){
      if(!isChoiceTask(task))return null;
      const meta=value?._integrity;
      if(meta?.v!==1)return {kind:'review',label:'Nicht verifizierbar',legacy:true};
      const chosen=[...(meta.selectedIds||[])].sort();
      const expected=[...(meta.expectedIds||[])].sort();
      const correct=chosen.length===expected.length&&chosen.every((v,n)=>v===expected[n]);
      const red=(meta.redFlagIds||[]).some(id=>chosen.includes(id));
      return {kind:red?'danger':correct?'correct':'wrong',label:red?'Red Flag':correct?'Richtig':'Falsch',legacy:false};
    },
    selectedText(value){
      const meta=value?._integrity;
      return meta?.v===1&&Array.isArray(meta.selectedTexts)?meta.selectedTexts.join(' · '):null;
    },
    expectedText(value){
      const meta=value?._integrity;
      return meta?.v===1&&Array.isArray(meta.expectedTexts)?meta.expectedTexts.join(' · '):null;
    }
  };
})();
